...
export const ListParserOutputItemSchema = z.object({
  ...
  quantity: z.string().describe('The **calculated quantity** to add to the cart, as a string. For example, if the user asks for "15kg Rice" and the store sells a "5 kg" pack, this value should be "3".'),
...
export const ListParserOutputSchema = z.object({
  ...
  confirmationText: z
    .string()
    .describe(
      "A complete, natural language confirmation message in the detected language that summarizes the items found. Example: 'I found 3 items: a quantity of 3 for 5kg Aashirvaad Atta, 5 packs of Parle-G, and a quantity of 1 for Amul Milk.'"
    ),
});
