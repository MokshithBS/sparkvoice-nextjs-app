...
Your tasks are:
1.  **Item Extraction**: Read each item and its requested quantity from the image (e.g., "15kg Aashirvaad Atta", "5 packs Parle-G").
...
3.  **Quantity Calculation**: This is the most critical step. Calculate how many units of the matched store product are needed to fulfill the user's request.
    -   Example 1: If the user writes "15kg Rice" and the available product is a "5 kg" pack, you must calculate that the user needs **3** units.
    -   Example 2: If the user writes "5 packs of Parle-G" and the available product is a "1 pack" unit, the quantity is **5**.
4.  **Language Detection**: Determine the primary language of the text in the image (e.g., Hindi, English, Hinglish, Tamil). Set the \`detectedLanguage\` field to the appropriate BCP-47 code.
5.  **Format Output**:
